...
# Decide onde o arquivo .db deve ficar: ao lado do .exe quando empacota,
...
# Simples e sem dependência externa - mais seguro do que guardar a
# senha em texto puro, mesmo sem ser um esquema de nível produção
# (que normalmente usaria salt + bcrypt/argon2).
def gerar_hash_senha(senha):
    return hashlib.sha256(senha.encode("utf-8")).hexdigest()

# Chamada e acesso ao banco de dados: usuários cadastrados + credenciais de login.
class Database:
    # Construtor
    ...
    # Verifica se o usuário e senha batem com o hash salvo no banco
...
class TelaLista(ctk.CTkToplevel):
    ...
    def carregar_dados(self):
        for i in self.tree.get_children():
            self.tree.delete(i)
        for row in self.db.get_all_user():
